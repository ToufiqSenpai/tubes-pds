...
from data.dataset import get_books, get_book_categories
...
df_books = df_books.merge(categories[['slug', 'title']], left_on='category_slug', right_on='slug', how='left', suffixes=('', '_category'))
df_books = df_books.rename(columns={'title_category': 'category_title'})
...
top5_cat_slugs = df_books["category_slug"].value_counts().head(5).index
filtered_cat = df_books[df_books["category_slug"].isin(top5_cat_slugs)]

# Get category titles for labels
top5_cat_titles = [filtered_cat[filtered_cat["category_slug"] == slug]["category_title"].iloc[0] if len(filtered_cat[filtered_cat["category_slug"] == slug]) > 0 else slug for slug in top5_cat_slugs]

fig6, ax6 = plt.subplots(figsize=(10, 5))
data_boxplot = [filtered_cat[filtered_cat["category_slug"] == c]["final_price"].dropna() for c in top5_cat_slugs]

# Properti Boxplot standard (Hitam/Putih aman)
ax6.boxplot(data_boxplot, labels=top5_cat_titles)
